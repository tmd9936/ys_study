...
select 
    ...
    job.job_title
from 
    employees emp, departments dep, jobs job
where 
    emp.department_id = dep.department_id
...
select 
    ...
    loc.city
from 
    employees emp, departments dep, jobs job, locations loc
where 
    emp.department_id = dep.department_id
    and emp.job_id = job.job_id
...
from 
    employees emp, locations loc, jobs job, departments dep
where 
...
select 
    emp1.employee_id 사원id, 
    emp1.first_name 사원이름, 
    emp2.employee_id 상사id, 
    emp2.first_name 상사이름
from 
    employees emp1, employees emp2
