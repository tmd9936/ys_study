...
def board_view(idx):
    ...
    if idx is not None:
        ...
        if data is not None:
            ...
            return render_template("view.html", result = result, search = search, keyword = keyword, page=page)
...
def board_write():
    if request.method == "POST":
        ...
        return redirect(url_for("boards.board_view", idx=doc.inserted_id))
    else :
    ...
    if request.method == "GET":
        ...
        if data is None:
            ...
            if session.get("id") == data.get("writer_id"):
                ...
                flash('글 수정 권한이 없습니다.')
                return redirect(url_for("boards.board_view", idx=idx))
